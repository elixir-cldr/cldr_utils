defmodule Cldr.Math do
  ...
  * `options` is a keyword list of options.
  ...
  defp do_convergents_with_limit([a | rest], {p_curr, q_curr}, {p_prev, q_prev}, max_denom, acc) do
    p_next = a * p_curr + p_prev
    q_next = a * q_curr + q_prev

    if  q_next > max_denom do
      # If next convergent exceeds limit, try semi-convergents
      semi_convergents = generate_semi_convergents(
        {p_prev, q_prev},
        {p_curr, q_curr},
        a,
        max_denom
  ...
  # Semi-convergents provide better approximations when the next full convergent
  # would exceed the denominator limit.
  ...
  # Returns a list of coefficients [a0, a1, a2, ...].
  ...
  * `{numerator, denominator}` is the approximate ratio for the float
  ...
  """
  def approximation_error(original, {num, denom}) do
    abs(original - num / denom)
  end
